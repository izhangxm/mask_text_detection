...
class SubM(Thread):
    ...
    def run(self) -> None:
        while True:
            # 检查是否时被终止
            if self.M._xjp2021_status == 4:
                break
            task_info = None
            try:
                task_info = self.M._xjp2021_task_queen.get(timeout=10)
                func_name = task_info['func_name']
                args, kwargs = task_info['params']
                f_event = task_info['f_event']
                task_id = task_info['task_id']
                worker_func = getattr(self.M._xjp2021_worker, func_name)
                result_data = worker_func(*args, **kwargs)
                self.M._xjp2021_result_dict[task_id] = result_data
                f_event.set()

            except Exception as e:
                if task_info:
                    self.M._xjp2021_result_dict[task_info['task_id']] = e
                    task_info['event'].set()
                else:
                    continue
...
class XJParallels(object):
    ...
    def __init__(self, worker, pd_num=1, sync=True):
        ...
        self._xjp2021_worker = worker

        self._xjp2021_status = 1  # 程序状态
        self._xjp2021_task_queen = Queue(maxsize=-1)  # 任务队列
        self._xjp2021_result_dict = {}  # 任务结果存贮字典
        self._xjp2021_task_dict = {}  # 任务结果存贮字典
    ...
    def _xjp2021_worker_func_decorator(self, attr_name):
        _worker_attr = getattr(self._xjp2021_worker, attr_name)
    ...
    def __getattr__(self, item):
        if hasattr(self._xjp2021_worker, item):
...
class XJPPool(object):
    def __init__(self, xjp_num, sync=False):
        self.ava_queen = Queue()
        self.xjp_num = xjp_num
        self.sync = sync
        self.pop_xpg_eve = Event()
...
class TInstance(object):
    ...
    def say_hello(self, ddd=12):
        time.sleep(3)
        print("hello, I am TInstance", ddd)
        return ddd
...
def test01():
    ...
    with xjp_woker.async_result():
        tid1, _ = xjp_woker.muu_sleep('muu_sleep async 3', 3)
        tid2, _ = xjp_woker.muu_sleep('muu_sleep async 1', 1)

        # print()
        xjp_woker.get_result(tid2)

        task_id, f_event = xjp_woker.say_hello('hahah')
